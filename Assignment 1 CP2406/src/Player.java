...
public class Player {
    ...
    {
        return hand.get(x);
    }       //Make a getter into it

    public void drawCard(Card card)
    {
        hand.add(card);
    }       //Method to put the card into the people

    public void getTurn(TableGame table)                    //Every action during their turn or change turn
    {
        int cardNum;
        boolean next = false;
        ...
        if(table.anyCardPlayed() && !(table.playerGetTurnAgain(this))) {        //Conditioning to see if hedidn't get his turn again twice while in game
            ...
            lastCardDescription =
                    "\nLast card = Name: " + table.getLastCard().getName() + "   " +
                            ...
                            //Description for normal card if they choose
            else{
                lastCardDescription = "\nPrevious player play the " +((SuperTrumpCard) table.getLastCard()).getName()+ " card\n";

            }
        }
        else if(table.anyCardPlayed() && table.playerGetTurnAgain(this)) {  // if anyone except if have passed the loop will came this part
            ...
            startGame(table);               //Allow player to choose the trump mode again
        }
        else                    //Conditioning if it is the start of the game
        {
            lastCardDescription = "No cards have been played";
            startGame(table);               //Allow the first player to pick the mode
        }
        while (!next) {             //Looping till the person did action that allow the next player to move
            ...
            if (choice.toUpperCase().equals("PASS")) {
                ...
                try {
                    ...
                    if(table.getCategoryMode().equals("NEW"))           //Decision if a special SuperTrump card that allow player to pick again the trump mode is played
                    {   table.putCard(cardplayed);
                        hand.remove((cardNum));
                        startGame(table);                               //Repick mode
